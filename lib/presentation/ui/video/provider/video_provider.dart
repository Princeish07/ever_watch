...
class VideoProvider extends StateNotifier<VideoState>{
  ...
  VideoProvider({this.videoRepository}):super(VideoState(videoListResult: Resource.loading(),isPlaying: true));

  void getVideoList() {
    videoRepository?.getVideoListStream()?.listen((Resource<List<VideoModel>> videoList) {
  ...
  likingVideo({String? id}) async{
    ...
    List<VideoModel>? updatedVideoList = state.videoListResult?.data?.map((video) {
      if (video.id == id) {
   ...
   state = state.copyWith(videoListResult: Resource.success(data: updatedVideoList));
  }
